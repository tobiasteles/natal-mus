"use client";

import { useState } from "react";
import ChildCard from "./ChildCard";

export default function ChildrenGallery() {
  const [page, setPage] = useState(1);
  const pageSize = 6;

  const children = [
    {
      name: "Amanda",
      gift: "Kit Maquiagem Infantil",
      photo: "/criancas/Amanda_Oliveira.jpeg",
      letter: "/Cartinhas-2025/Amanda_Oliveira.jpg",
    },
    {
      name: "Benjamim",
      gift: "Carrinho de controle remoto",
      photo: "/criancas/Benjamim_Sousa.JPG",
      letter: "/Cartinhas-2025/Benjamim_Sousa.jpg",
    },
    {
      name: "Calebe",
      gift: "Camera Fotográfica Infantil",
      photo: "/criancas/Calebe_Rodrigues.JPG",
      letter: "/Cartinhas-2025/Calebe_Rodrigues.jpg",
    },
    {
      name: "Davi Barbosa",
      gift: "Carrinho de controle remoto",
      photo: "/criancas/Davi_Barbosa.JPG",
      letter: "/Cartinhas-2025/Davi_Barbosa.jpg",
    },
    {
      name: "Laylah Beatriz",
      gift: "Boneca",
      photo: "/criancas/Laylah_Beatriz.JPG",
      letter: "/Cartinhas-2025/Laylah_Beatriz.jpg",
    },
    {
      name: "Maria Eduarda",
      gift: "Boneca Barbie",
      photo: "/criancas/Maria_Eduarda.JPG",
      letter: "/Cartinhas-2025/Maria_Eduarda.jpg",
    },
    {
      name: "Catarina Bastos",
      gift: "Roupa 7anos / Bebê Reborn",
      photo: "/criancas/Catarina.jpg",
      letter: "/Cartinhas-2025/Catarina.jpg",
    },
    {
      name: "Cibelly de Carvalho",
      gift: "Bebê Reborn",
      photo: "/criancas/Cibelly_Carvalho.JPG",
      letter: "/Cartinhas-2025/Cibelly_Carvalho.jpg",
    },
    {
      name: "Cindy Alexia",
      gift: "BobieGoods / Stitch",
      photo: "/criancas/Cindy.jpeg",
      letter: "/Cartinhas-2025/Cindy.jpg",
    },
    {
      name: "Davi Felipe",
      gift: "Câmera fotográfica infantil/Coleção Diário de um Banana",
      photo: "/criancas/Davi_Felipe.JPG",
      letter: "/Cartinhas-2025/Davi_Felipe.jpg",
    },
    {
      name: "Davi Henrique",
      gift: "Homem Aranha/Naruto/Carrinho controle remoto",
      photo: "/criancas/Davi Henrique.jpg",
      letter: "/Cartinhas-2025/Davi_Henrique.jpg",
    },
    {
      name: "Davi Lucca",
      gift: "Arco e flecha/Robô",
      photo: "/criancas/Davi_Lucca.JPG",
      letter: "/Cartinhas-2025/Davi_Lucca.jpg",
    },
    {
      name: "Deynner Jesus",
      gift: "Carrinho corrida/ Tenis Homem Aranha 29",
      photo: "/criancas/Deynner.jpeg",
      letter: "/Cartinhas-2025/Deynner.jpg",
    },
    {
      name: "Emanoel Evangelista",
      gift: "Uniforme Cruzeiro",
      photo: "/criancas/Emanoel.JPG",
      letter: "/Cartinhas-2025/Emanoel.jpg",
    },
    {
      name: "Ester Ferreira",
      gift: "Bicicleta 4 anos/Barbie",
      photo: "/criancas/Ester_Ferreira.JPG",
      letter: "/Cartinhas-2025/Ester_Ferreira.jpg",
    },
    {
      name: "Isabella Sousa",
      gift: "Bebe Reborn/Patins 31-32",
      photo: "/criancas/Isabella_Sousa.JPG",
      letter: "/Cartinhas-2025/Isabella_Sousa.jpg",
    },
    {
      name: "Isabelly Batista",
      gift: "Roupa 10 anos/Kit Escolar",
      photo: "/criancas/Isabelly_batista.jpg",
      letter: "/Cartinhas-2025/Isabelly_Batista.jpg",
    },
    {
      name: "Jhennifer Gabrielly",
      gift: "Kit Escolar/ Patins 35-36",
      photo: "/criancas/Jennyfer.jpeg",
      letter: "/Cartinhas-2025/Jennyfer.jpg",
    },
    {
      name: "José Arthur",
      gift: "Tênis Nike/Uniforme Santos",
      photo: "/criancas/Jose_Arthur.jpeg",
      letter: "/Cartinhas-2025/Jose_Arthur.jpg",
    },
    {
      name: "Jesus Alerrandro",
      gift: "Beyblade/Skate",
      photo: "/criancas/Jesus_Alerandro.JPG",
      letter: "/Cartinhas-2025/Jesus_Alerandro.jpg",
    },
    {
      name: "Júlio Cesar",
      gift: "Tênis Nike/Uniforme Santos",
      photo: "/criancas/Julio.JPG",
      letter: "/Cartinhas-2025/Julio.jpg",
    },
    {
      name: "Lorenna Souza",
      gift: "Stitch/Cozinha/Boneca",
      photo: "/criancas/Lorenna_Sousa.jpeg",
      letter: "/Cartinhas-2025/Lorenna_Sousa.jpg",
    },
    {
      name: "Maria Fernanda",
      gift: "Kit cozinha/Kit médico",
      photo: "/criancas/Maria_Fernanda.JPG",
      letter: "/Cartinhas-2025/Maria_Fernanda.jpg",
    },
    {
      name: "Maria Helena",
      gift: "Barbie Sereia/Kit Pulseira",
      photo: "/criancas/Maria_Helena.jpg",
      letter: "/Cartinhas-2025/Maria_Helena.jpg",
    },
    {
      name: "Miguel da Silva",
      gift: "Beyblade/Cartinhas Pokemon",
      photo: "/criancas/Miguel_da_Silva.jpg",
      letter: "/Cartinhas-2025/Miguel_da_Silva.jpg",
    },
    {
      name: "Nicolas Samuel",
      gift: "Carretilha Pipa/Cartinhas Pokemon",
      photo: "/criancas/Nicolas_Samuel.JPG",
      letter: "/Cartinhas-2025/Nicolas_Samuel.jpg",
    },
    {
      name: "Sarah Emanoele",
      gift: "Kit Pulseira Barbie",
      photo: "/criancas/Sarah_Emanoele.JPG",
      letter: "/Cartinhas-2025/Sarah_Emanoele.jpg",
    },
    {
      name: "Stella Mota",
      gift: "Patinete/Boneca",
      photo: "/criancas/Stella_Mota.jpg",
      letter: "/Cartinhas-2025/Stella_Mota.jpg",
    },
    {
      name: "Talia Emanuelly",
      gift: "Bebe Reborn/Patins 31/37-38",
      photo: "/criancas/Talia_Emanuelly.jpg",
      letter: "/Cartinhas-2025/Talia_Emanuelly.jpg",
    },
  ];

  const start = (page - 1) * pageSize;
  const currentChildren = children.slice(start, start + pageSize);
  const totalPages = Math.ceil(children.length / pageSize);

  return (
    <section id="children" className="py-20 bg-white">
      <div className="container mx-auto px-4">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-black text-[#0a28e6] mb-4">
              Conheça Nossos Alunos
            </h2>
            <p className="text-xl text-gray-600 max-w-2xl mx-auto font-medium">
              Apoie o desenvolvimento e os sonhos das crianças da Estrutural através da educação e do cuidado <span className="text-[#6e55f5] font-bold">em 2026</span>.
            </p>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
            {currentChildren.map((child, index) => (
              <ChildCard key={index} {...child} />
            ))}

            {/* Paginação Estilizada */}
            <div className="flex justify-center items-center gap-6 mt-12 w-full col-span-full">
              <button
                disabled={page === 1}
                onClick={() => setPage(page - 1)}
                className="px-6 py-2 rounded-full border-2 border-[#0a28e6] text-[#0a28e6] font-bold 
                           hover:bg-[#0a28e6] hover:text-white transition-all 
                           disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-[#0a28e6]"
              >
                Anterior
              </button>

              <div className="flex items-center gap-2">
                <span className="flex items-center justify-center w-12 h-12 rounded-full bg-[#99de81] text-[#0a28e6] font-black shadow-md">
                  {page}
                </span>
                <span className="text-gray-400 font-bold">/</span>
                <span className="text-gray-500 font-bold">{totalPages}</span>
              </div>

              <button
                disabled={page === totalPages}
                onClick={() => setPage(page + 1)}
                className="px-6 py-2 rounded-full border-2 border-[#0a28e6] text-[#0a28e6] font-bold 
                           hover:bg-[#0a28e6] hover:text-white transition-all 
                           disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-[#0a28e6]"
              >
                Próxima
              </button>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}